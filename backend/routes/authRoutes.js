...
router.post('/forgot-password', async (req, res) => {
    try {
        ...
        // Find user (don't reveal existence for security)
        const user = await User.findOne({ email: email.toLowerCase() });
        ...
        // For now, just log the token (email sending will be added later)
        console.log(`🔑 Reset token for ${email}: ${resetToken}`);
        console.log(`🔗 Reset link: ${process.env.CLIENT_URL}/reset-password?token=${resetToken}&email=${email}`);
